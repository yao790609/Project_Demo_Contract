## 專案摘要
本作品集展示我以 Python、PostgreSQL、Excel 與 Grafana 建置資料擷取、整理、分析、查詢與視覺化流程的實作經驗，內容涵蓋資料管理架構演進、分析邏輯設計、報表產出與視覺化展示。
...
<br>
本作品集主要展示我如何運用 Python、Excel、PostgreSQL 與 Grafana 建置資料處理與分析流程，內容包含系統運算邏輯概述，以及系統程式架構與實作說明。文中附件所提供之檔案，主要為早期以 Excel 作為資料管理架構時所保存的市場原始資料，以及透過 Python 計算後產出的結果報表；後續系統調整為以 PostgreSQL 搭配 SQL 指令進行資料處理與結果產出，因此現行架構下較無相同形式之報表檔案可供附上。文中所展示之程式碼並非完整系統全部內容，主要用於說明整體設計思路與相關技術能力。<br>
...
# 自身能力評估
1. Python 與資料處理能力：可獨立使用 Python 進行資料擷取、資料清洗、資料庫存取、演算法設計、報表設計與產出等流程，具備從資料蒐集、處理到結果輸出的完整實作經驗。<br>
2. 資料分析與視覺化能力：熟悉使用 Python 進行資料清洗、分析與視覺化，並具備 NumPy、Pandas 等資料處理工具實務經驗。早期以 matplotlib 進行圖表呈現，後續配合資料架構調整，亦搭配 Grafana dashboard 視覺化展示，以提升資料檢視與應用效率。<br>
...
## :spider:資料搜集
爬蟲上市/上櫃所有個股資料，包含以下項目，在交易日的 14:35 系統會開始自動運作！<br>
- 每日收盤行情<br>
- 三大法人買賣超日報<br>
- 融資融券餘額<br>
- 融券借券賣出餘額<br>
