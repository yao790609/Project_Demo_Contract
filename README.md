:wave: 您好，我是對資料分析與系統應用具有高度興趣的姚廷諺。自 2016 年起，我開始自學 Python，並將其應用於股票市場相關資料的整理、分析與策略邏輯實作，逐步累積資料清理、整併、儲存、報表設計、視覺化呈現與系統建置等經驗。早期資料管理主要以 Excel 建立與維護，並透過 Python 進行資料讀寫、整理與分析；隨著資料量與分析需求增加，目前已調整為以 PostgreSQL 作為主要資料儲存架構，並搭配 Python 進行資料擷取、查詢、整併、分析與視覺化處理。<br>

本作品集主要展示我如何運用 Python、Excel 與 PostgreSQL 建置資料處理與分析流程，內容分為兩部分：第一部分為系統運算邏輯概述，第二部分為系統程式架構與實作說明。文中所展示之程式碼並非完整系統全部內容，主要用於說明整體設計思路與相關技術能力。
...
# 自身能力評估
1. Python與資料處理能力：可獨立使用Python進行資料擷取、資料清洗、整併、資料庫存取、報表設計與產出，以及圖表視覺化等流程，具備從資料蒐集、處理到結果輸出的完整實作經驗。<br>
2. 資料分析與視覺化能力：熟悉使用Python進行資料清洗、分析與視覺化，並具備NumPy、Pandas等資料處理工具實務經驗，能將原始資料轉換為具邏輯性與可讀性的分析結果，方便後續檢視、比較與應用。<br>
3. Excel與報表製作能力：熟悉Excel資料整理、函數應用（如 VLOOKUP）、報表彙整與圖表製作，能依需求將資料整理為清楚易讀之表格、圖表與簡報素材，協助後續溝通與應用。<br>
4. 資料庫操作能力：熟悉PostgreSQL資料表設計、資料儲存、查詢與維護，並能透過Python連接資料庫進行資料擷取與應用，同時使用pgAdmin進行資料庫操作與管理。<br>
